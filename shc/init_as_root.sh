...
if [[ $# -eq 0 ]]; then
    set -- xpra desktop "${DISPLAY}" --start-child=fluxbox --exit-with-children=yes --daemon=no --bind-tcp=0.0.0.0:8080 --mdns=no --webcam=no --printing=no --systemd-run=no --ssh-upgrade=no --xvfb=Xdummy --start-late="terminator -e /initialize.sh"
fi
